...
printf "\n\n${bgreen}#######################################################################${reset}\n"
printf "${bgreen} Z-Recon installer/updater script ${reset}\n\n"
...
if [[ $(id -u | grep -o '^0$') == "0" ]]; then
    ...
    if sudo -n false 2>/dev/null; then
        ...
        printf "${bred} echo \"${USERNAME}  ALL=(ALL:ALL) NOPASSWD: ALL\" > /etc/sudoers.d/Z-Recon${reset}\n\n"
    fi
    SUDO="sudo"
fi
...
printf "${bblue} Running: Looking for new Z-Recon version${reset}\n\n"
...
then
    ...
    if [ -n "$(git status --porcelain | egrep 'Z-Recon.cfg$')" ]; then
        mv Z-Recon.cfg Z-Recon.cfg_bck
        printf "${yellow} Z-Recon.cfg has been backed up in Z-Recon.cfg_bck${reset}\n\n"
    fi
    eval git reset --hard $DEBUG_STD
    eval git pull $DEBUG_STD
    printf "${bgreen} Updated! Running new installer version...${reset}\n\n"
else
    printf "${bgreen} Z-Recon is already up to date!${reset}\n\n"
fi
